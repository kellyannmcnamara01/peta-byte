...
namespace PetaByte_KellysFeatures2.Controllers
{
    public class AlertsController : Controller
    {
        ...
        {
            //place action into a try catch
            try
            {
                if (ModelState.IsValid)
                {
                    //have the timestamp input the current date and time
        ...
        {
            //place ation into a try catch
            try
            {
                if (ModelState.IsValid)
                {
                    //have the timestamp update when the user edits the alert
                    alert.timestamp = DateTime.Now;
